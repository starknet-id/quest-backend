...
#[cfg(test)]
mod tests {
    use crate::{config::{self, Config}, logger};
    ...
    async fn setup_test_db() -> Database {
        ...
        db.collection::<Document>("completed_tasks").drop(None).await.ok();
    ...
    async fn test_get_quest_participants() {
        ...
        let app_state = Arc::new(AppState { db: db.clone(), last_task_id: Mutex::new(0), last_question_id: Mutex::new(0), conf, logger, provider });
    ...
    let body: Value = serde_json::from_slice(&body_bytes).unwrap();
        ...
        let quest_completion_timestamp = body["first_participants"][0]["completion_time"].as_i64().unwrap();
